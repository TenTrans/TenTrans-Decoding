                                           ...
                                           const int vocab_size)
{
    ...
    if(tid == 0)
    {
        ...
        if(finished_buf != nullptr)
        {
            ...
            {
                sequence_length[batch_id] = finished_buf[batch_id] ? sequence_length[batch_id] : sequence_length[batch_id] + 1;
            }
        }
    }
}
                         ...
                         const int vocab_size)
{
    ...
    if(tid < candidate_num)
    {
        ...
        topk_tmp_val_buf[bid * candidate_num + tid] = (T)__expf((float)topk_tmp_val_buf[bid * candidate_num + tid] - max_val);
    }
                                        ...
                                        const int batch_size)
{
    ...
    if(workspace == nullptr)
    {
        workspace_size = sizeof(int) * topk_tmp_ids_buf_size + sizeof(T) * topk_tmp_val_buf_size;
    }
    else
    {
...
template void topK_sampling_kernel_kernelLauncher_v2(void* workspace,
                                                    size_t& workspace_size,
                                                    float* log_probs,
                                                    int* ids,
                                                    int* sequence_length,
                                         ...
                                         const int batch_size)
{
    ...
    T* sorted_log_probs = (T*)((char*)cub_temp_storage + args.cub_temp_storage_size_);
                                            ...
                                            const int batch_size)
{
    ...
    {
        beam_topK_kernel_for_topP<T, 1, block_size><<<batch_size, block_size, 0, stream>>>(log_probs, \
            sorted_id_vals, sorted_log_probs, vocab_size, offset_buf,begin_offset_buf, args.probability_threshold_); 
...
template void topP_sampling_kernel_kernelLauncher_v2(void* workspace,
                                                     size_t& workspace_size,
                                                     const float* log_probs,
                                                     const int* id_vals,
                                                     int* offset_buf,
...
template void topK_topP_sampling_kernel_kernelLauncher_v2(void* workspace,
                                                          size_t& workspace_size,
                                                          int* output_ids,
                                                          const float* logits,
                                                          bool* finished_buf,
