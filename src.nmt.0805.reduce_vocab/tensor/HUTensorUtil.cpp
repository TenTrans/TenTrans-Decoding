...
HUShape HUTensorUtil::GetCopyColsShape(HUPtr<HUTensor> a, const std::vector<size_t>& indices) {
    int num = indices.size();
    HUShape shape = a->shape();
    ABORT_IF(shape.size() != 2, "[TenTrans] rows operator can only be used with 2-dimensional tensors");
    shape.set(1, num);
...
HUPtr<HUTensor> HUTensorUtil::CopyCols(HUPtr<HUTensor> in, const std::vector<size_t>& indices, HUPtr<HUMemPool> mem)
{
    auto outShape = GetCopyColsShape(in, indices);
    auto outMem = mem->alloc<TT_DATA_TYPE>(outShape.elements());
    auto out = HUNew<HUTensor>(outMem, outShape, in->getDevice());
    CopyColsOP(out, in, indices);
    return out;
}
