...
void HUBeamSearch::FreeTmpOutputLayerCache()
{
    auto tmpBias = this->encdec_->GetOutputLayerBias();
    this->memPool_->free(tmpBias->memory());
    auto tmpW = this->encdec_->GetOutputLayerW();
    this->memPool_->free(tmpW->memory());
}
...
HUHistories HUBeamSearch::Search(HUPtr<HUBatch> batch)
{
    ...
    */
    ...
    while(curStep < maxStep)
    {
        ...
        else
        {
            ...
            for (size_t sentId = 0; sentId < dimBatch; sentId++) {
                ...
                for (size_t beamId = 0; beamId < beamSize_; beamId++) {
                    ...
                    if ((this->useShortList_) && ((batch->newVocab()).size() > 0)) {
                        embIndices.push_back(batch->newVocab()[wordId]);
                    }
                    else {
                        embIndices.push_back(wordId);
                    }
        ...
        auto state = encdec_->Step(curState, hypIndicesDevice, hypIndices.size(), embIndices, beamSize_, dimBatch, 
                isAllDoneDevice, batch->newVocab(), isFirstStep);
        ...
        // std::cout << "dimTgtWords: " << dimTgtWords << std::endl;
    ...
    if ((this->useShortList_) && ((batch->newVocab()).size() > 0)) {
        this->FreeTmpOutputLayerCache();
        // std::cout << "Free tmp vocab ..." << std::endl;
    }
