  ...
  const int size_per_head)
{
  ...
  if(k <= 1024)
  {
    if(sizeof(T) == 4)
    {
      ...
      dim3 block(k);
      add_QKV_bias<T><<<grid, block>>>(Q, bias_Q, K, bias_K, V, bias_V, q_buf, k_buf, v_buf,
    ...
    else
    {
      ...
      add_QKV_bias<T><<<grid, block>>>(Q, bias_Q, K, bias_K, V, bias_V, q_buf, k_buf, 
  ...
  else
  {
    ...
    if(sizeof(T) == 4)
    {
      ...
      add_QKV_bias_generalized<T><<<grid, block>>>(Q, bias_Q, K, bias_K, V, bias_V, q_buf, k_buf, v_buf,
        batch_size, seq_len, head_num, size_per_head, word_per_block);
    ...
    else
    {
      ...
      assert(grid.x <= 65536 && grid.x > 0);
      add_QKV_bias_generalized<half2><<<grid, block>>>((const half2*)Q, (const half2*)bias_Q, 
  ...
  const int size_per_head)
{
  ...
  add_fusedQKV_bias_transpose_kernel<<<grid, block>>>(
  ...
  const T scalar)
{
  ...
  if (seq_len % 2 != 0){
    ...
    if(batch_size * head_num <= 120)
    {
      ...
      softmax_kernel_v2<T><<<grid, block>>>(buffer, attr_mask, batch_size, head_num, seq_len, scalar);
    }
    else
    {
      grid.x = batch_size * head_num;
      softmax_kernel<T><<<grid, block>>>(buffer, attr_mask, batch_size, head_num, seq_len, scalar);
    }
  }
  //deal with even seq_len 
    ...
    if (seq_len <= 32){
      block.x = 32;
      softmax_kernel_v3_LE32<T><<<grid, block>>>(buffer, attr_mask, batch_size, head_num, seq_len, scalar);
    }
    else{
      if (sizeof(T) == 2){
        block.x = (seq_len/2 + 31)/32*32;
        softmax_kernel_v3<<<grid, block>>>(buffer, attr_mask, batch_size, head_num, seq_len, scalar);
      }
      else{
        block.x = (seq_len + 31)/32*32;
        softmax_kernel_v3<T><<<grid, block>>>(buffer, attr_mask, batch_size, head_num, seq_len, scalar);
      }
    }
  ...
  const int size_per_head)
{
  ...
  if(sizeof(T) == 2)
  {
    ...
    transpose<T><<<grid, block>>>(src, dst, 
