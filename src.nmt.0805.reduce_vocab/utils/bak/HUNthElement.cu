...
__global__ 
void gMaxElementUpdate(float* binCosts, 
                       int* binIdxs, 
                       float* probs, 
                       int* batchFirstElements, 
                       float* outCosts, 
                       int* outIdxs, 
                       int* cummulatedBeamSizes, int NUM_BLOCKS) 
{
  extern __shared__ float sdata[];
  __shared__ int indices[512];
...
private:
                    ...
                    const std::vector<int>& cummulatedBeamSizes) 
  {
    ...
    gMaxElement<<<NUM_BLOCKS, BLOCK_SIZE, BLOCK_SIZE * sizeof(gMaxElement), 0>>>
        (d_out, d_ind, probs, numBatches, d_batchPosition);

    gMaxElementUpdate<<<numBatches, BLOCK_SIZE, BLOCK_SIZE * sizeof(float), 0>>>
...
private:
  ...
  int* d_res_idx;      // device data, result index
  float* d_res;        // device data, result

  int* h_res_idx;      // host data, result index
  float* h_res;        // host data, result
