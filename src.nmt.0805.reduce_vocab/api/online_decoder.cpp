...
void OnlineDecoder::Init(std::string configPath)
{
    ...
    /*
    auto useShortList = this->config_->get<bool>("use-shortlist");
    if (useShortList)
    {
        auto shortList = this->config_->get<std::string>("shortlist-path");
        this->shortList_ = cnpy::npz_load(shortList);
    }
    else
    {   auto shortList = this->config_->get<std::string>("shortlist-path");
        this->shortListNpz_ = cnpy::npz_load(shortList);
    }
    */
    ...
    bool useShortList = this->config_->get<bool>("use-shortlist");
    this->search_ = HUNew<HUBeamSearch>(this->config_, this->encDec_, beamSize, earlyStop, useShortList, 
            EOS_ID, UNK_ID, this->memPool_, this->device_);
}
...
void OnlineDecoder::DoJob(std::vector<string> &inputs, std::vector<string> &outputs)
{
    ...
    int totalTokenNum = 0;
    HUPtr<HUTextInput> in = HUNew<HUTextInput>(inputs, this->srcVocab_ /*, this->shortListNpz_*/);
    HUPtr<HUBatch> batch = in->ToBatch(in->ToSents(), this->shortList_);
